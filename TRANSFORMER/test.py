...
class Embedding(nn.Module):
    ...
    def __init__(self, features_num, embedding_dim, max_len=512):
        """
        词嵌入类，支持共享词嵌入和位置编码。
...
class Decoder(nn.Module):
    ...
    def __init__(self, features_num, embedding_dim, num_layers, num_heads, d_ff, max_len=512, dropout=0.1, shared_weight=None):
...
# ----------- 训练函数 -----------  
def train_transformer(model, train_data, embedding_dim, num_epochs=10, lr=0.0001):
    criterion = nn.MSELoss()  # 连续值预测任务
    optimizer = optim.Adam(model.parameters(), lr=lr)

    # 将模型移动到 GPU
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.to(device)  # 模型移动到 GPU（如果可用）
    ...
    for epoch in range(num_epochs):
        ...
        for src, tgt in train_data:
            ...
            # 生成掩码并确保掩码在 GPU 上
            src_mask = torch.ones(src.size(0), 1, 1, src.size(1)).to(torch.bool).to(device)
            tgt_mask = model.generate_tgt_mask(tgt.size(1)).to(torch.bool).to(device)

            optimizer.zero_grad()
            output = model(src, tgt, src_mask, tgt_mask)

            # 标签应与输出同维度，即 [batch, tgt_len, embedding_dim]
            target_label = torch.rand_like(output).to(device)  # 假设你已有 ground truth，这里先造一个
